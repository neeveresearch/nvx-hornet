...
The core module is self-contained. The HK2 module is a plugin for teams that want enterprise dependency injection (think: "Spring-lite for Neeve apps"). Most of the interesting stuff lives in the core.
...
| Technology | Why |
|---|---|
| **nvx-talon (AEP Engine)** | The Neeve platform's core — provides the actual messaging, clustering, and event sourcing infrastructure |
...
| **ADM (Advanced Data Model)** | Neeve's own message serialization — optimized for zero-copy, zero-garbage messaging |
...
4. **The framework trusts its platform.** nvx-hornet doesn't try to abstract away the AEP engine — it embraces it. The `TopicOrientedApplication` interface exposes engine lifecycle hooks directly. This tight coupling to nvx-talon is intentional: the framework is a productivity layer over the platform, not a platform-agnostic abstraction.
